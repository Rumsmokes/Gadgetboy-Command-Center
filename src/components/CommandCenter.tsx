...
import DailyOperationsWindow from './DailyOperationsWindow';
...
export default function CommandCenter(props: Props) {
  const [data, setData] = useState({ customers: [] as any[], technicians: [] as any[], workOrders: [] as any[], sales: [] as any[], calendarEvents: [] as any[], calendarNotes: [] as any[], purchaseOrders: [] as any[], attentionSettings: undefined as any, cloverDailyTotals: {} as Record<string, unknown> });
  ...
  const [attentionFilter, setAttentionFilter] = useState<'all' | 'workorder' | 'sales' | 'money'>('all');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({ customers: customers || [], technicians: technicians || [], workOrders: reconciledWorkOrders, sales: sales || [], calendarEvents: calendarEvents || [], calendarNotes: calendarNotes || [], purchaseOrders: purchaseOrders || [], attentionSettings, cloverDailyTotals: settingsRows?.[0]?.cloverDailyTotals || {} });
  ...
  const refreshCollection = useCallback(async (key: string) => {
    ...
    if (key === 'settings') return setData(current => ({ ...current, attentionSettings: rows?.[0]?.ticketCleanupSettings, cloverDailyTotals: rows?.[0]?.cloverDailyTotals || {} }));
  ...
  const attentionAudit = useMemo(() => buildAttentionAudit(panelRecords), [panelRecords]);
  const filteredAttentionGroups = useMemo(() => attentionAudit.groups.map((group:any) => ({ ...group, entries: group.entries.filter((entry:any) => attentionFilter === 'all' || (attentionFilter === 'money' ? entry.group === 'Money & inventory review' : entry.record.kind === attentionFilter) ) })).filter((group:any) => group.entries.length), [attentionAudit, attentionFilter]);
  ...
  return <div className="command-center">
    ...
    <div className="command-center-metrics">
      ...
      <button onClick={() => showRecords('Collected Today', model.records.filter(row => new Date(row.activityAt).toDateString() === new Date().toDateString()))} title={model.cloverReconciliation.cloverTotal == null ? 'Enter the Clover total in EOD Report to reconcile today.' : `Clover ${money(model.cloverReconciliation.cloverTotal)} · ${model.cloverReconciliation.reconciled ? 'reconciled' : `difference ${money(Math.abs(model.cloverReconciliation.variance))}`}`}><span>Collected today</span><strong>{money(model.collectedToday)}</strong><small>{model.cloverReconciliation.cloverTotal == null ? `${model.paymentsToday} payments · Clover not entered` : model.cloverReconciliation.reconciled ? `Clover matched · ${model.paymentsToday} payments` : `Clover difference ${money(Math.abs(model.cloverReconciliation.variance))}`}</small></button>
    </div>
    ...
    {panel && panel.kind !== 'statistics' && panel.kind !== 'product-delivery' ? <div className="command-center-panel-layer" onMouseDown={event => { if (event.target === event.currentTarget) setPanel(null); }}><section className="command-center-panel"><header><h2>{panel.title}</h2><div><button title="Open in separate window" onClick={() => window.open(window.location.href, '_blank', 'width=1100,height=800')}>↗</button><button aria-label="Close" onClick={() => setPanel(null)}>×</button></div></header>{panel.title === 'Needs Attention' ? <div className="command-center-attention-audit" data-testid="attention-audit"><p className="command-center-attention-scope">Audit desk · invoices created September 1, 2026 or later</p><div className="command-center-attention-filters"><button className={attentionFilter==='all'?'active':''} onClick={()=>setAttentionFilter('all')}>All issues</button><button className={attentionFilter==='workorder'?'active':''} onClick={()=>setAttentionFilter('workorder')}>Work orders</button><button className={attentionFilter==='sales'?'active':''} onClick={()=>setAttentionFilter('sales')}>Sales</button><button className={attentionFilter==='money'?'active':''} onClick={()=>setAttentionFilter('money')}>Money & inventory</button></div><div className="command-center-attention-metrics"><span><strong>{attentionAudit.metrics.urgent + model.globalAttention.length}</strong>Urgent</span><span><strong>{attentionAudit.metrics.money}</strong>Money & inventory</span><span><strong>{attentionAudit.metrics.communication}</strong>Ticket details</span><span><strong>{attentionAudit.metrics.completeness}%</strong>Complete</span></div>{model.globalAttention.length ? <section className="command-center-attention-group"><header><strong>Payment reconciliation</strong><small>{model.globalAttention.length} open</small></header>{model.globalAttention.map(issue => <article className="command-center-attention-row urgent" key={issue.code}><i/><div><strong>Clover variance · {money(Math.abs(issue.amount))}</strong><small>{issue.label}. POS {money(model.cloverReconciliation.posTotal)} · Clover {money(model.cloverReconciliation.cloverTotal || 0)}</small></div><button onClick={() => props.onOpenModal('eod')}>Open EOD</button></article>)}</section> : null}{filteredAttentionGroups.map((group:any)=><section className="command-center-attention-group" data-testid="attention-group" key={group.group}><header><strong>{group.group}</strong><small>{group.entries.length} open</small></header>{group.entries.map((entry:any)=><article className={`command-center-attention-row ${entry.priority}`} key={`${entry.record.kind}-${entry.record.id}`} onContextMenu={event=>openRecordMenu(event,entry.record)}><i/><CommandCenterRecordHoverCard record={entry.record}><strong>{entry.record.kind==='workorder'?`WO #${entry.record.id}`:`Invoice #${entry.record.id}`} · {recordLabel(entry.record)}</strong><small>{entry.record.customerName} · {entry.record.problem || entry.record.title}</small></CommandCenterRecordHoverCard><div><strong>{entry.detail}</strong><small>{entry.record.stage || entry.record.status} · {money(entry.record.remaining)}</small></div><button onClick={()=>activateRecord(entry.record)}>{entry.action}</button></article>)}</section>)}{!filteredAttentionGroups.length && !model.globalAttention.length?<p className="command-center-empty">No in-scope records need attention.</p>:null}</div> : panel.kind === 'today' ? <div className="command-center-today-preview">{((panel as any).records || []).map((row: any, index: number) => <button type="button" key={String(row?.id || index)} onClick={()=>openTodayItem(row)}><strong>{todayLabel(row)}</strong><span>{todayDetail(row)}</span><time>{todayTime(row)}</time></button>)}</div> : <div className="command-center-panel-table"><table><thead><tr><th>Record</th><th>Device / Client</th><th>Status</th><th>Technician</th><th>Balance</th><th>Activity</th></tr></thead><tbody>{panelRecords.map(record => <tr className={record.expedited ? 'expedited' : ''} key={`${record.kind}-${record.id}`} onDoubleClick={() => activateRecord(record)} onContextMenu={event => openRecordMenu(event, record)} {...longPressHandlers(record)}><td>{record.kind === 'workorder' ? `WO #${record.id}` : `Invoice #${record.id}`}</td><td><CommandCenterRecordHoverCard record={record} className="command-center-panel-record"><strong>{recordLabel(record)}</strong><small>{record.customerName}</small></CommandCenterRecordHoverCard></td><td>{record.stage || record.status}</td><td>{record.technician}</td><td>{money(record.remaining)}</td><td>{relativeAge(record.activityAt)}</td></tr>)}</tbody></table></div>}{panel.title !== 'Needs Attention' && !((panel as any).records || []).length ? <p className="command-center-empty">Nothing scheduled in this category today.</p> : null}</section></div> : null}
    {dailyOperationsOpen ? <DailyOperationsWindow onClose={() => setDailyOperationsOpen(false)} /> : null}
