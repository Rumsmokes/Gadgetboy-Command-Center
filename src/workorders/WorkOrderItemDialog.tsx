...
export default function WorkOrderItemDialog({ item, onClose, onSave }: {
  ...
  async function save() {
    ...
    if (hasPart && source === 'order' && addToCart && (!Number.isFinite(Number(draft.internalCost)) || Number(draft.internalCost) < 0 || !String(draft.distributor || '').trim() || !String(draft.orderSourceUrl || '').trim())) {
      setError('Supplier cost, supplier, and order URL are required before adding this part to the EOD cart.'); return;
    }
    ...
    const next: WorkOrderItemRow = {
      ...
      requiresOrder: hasPart && source === 'order' && addToCart,
      orderStatus: hasPart && source === 'order' && addToCart ? 'needed' : 'in_stock',
      orderDate: hasPart && source === 'order' && addToCart ? '' : draft.orderDate,
      estimatedDeliveryDate: hasPart && source === 'order' && addToCart ? '' : draft.estimatedDeliveryDate,
